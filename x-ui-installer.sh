...
PACKAGE_UPDATE=("sudo apt-get update" "sudo apt-get update" "sudo yum -y update" "sudo yum -y update" "sudo yum -y update" "sudo apk update -f")
PACKAGE_INSTALL=("sudo apt -y install" "sudo apt -y install" "sudo yum -y install" "sudo yum -y install" "sudo yum -y install" "sudo apk add -f")
PACKAGE_REMOVE=("sudo apt -y remove" "sudo apt -y remove" "sudo yum -y remove" "sudo yum -y remove" "sudo yum -y remove" "sudo apk del -f")
PACKAGE_UNINSTALL=("sudo apt -y autoremove" "sudo apt -y autoremove" "sudo yum -y autoremove" "sudo yum -y autoremove" "sudo yum -y autoremove" "sudo apk del -f")

#[[ $EUID -ne 0 ]] && red "Please run the script as the root user" && exit 1

CMD=("$(grep -i pretty_name /etc/os-release 2>/dev/null | cut -d \" -f2)" "$(sudo hostnamectl 2>/dev/null | grep -i system | cut -d : -f2)" "$(sudo lsb_release -sd 2>/dev/null)" "$(grep -i description /etc/lsb-release 2>/dev/null | cut -d \" -f2)" "$(grep . /etc/redhat-release 2>/dev/null)" "$(grep . /etc/issue 2>/dev/null | cut -d \\ -f1 | sed '/^[ ]*$/d')")
...
for ((int = 0; int < ${#REGEX[@]}; int++)); do
    [[ $(echo "$SYS" | sudo tr '[:upper:]' '[:lower:]') =~ ${REGEX[int]} ]] && SYSTEM="${RELEASE[int]}" && [[ -n $SYSTEM ]] && break
done
...
install() {
    sudo bash <(curl -Ls https://raw.githubusercontent.com/NidukaAkalanka/x-ui-english/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            sudo start
        else
